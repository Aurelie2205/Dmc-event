exports.handler = async (event) => {
  ...
  // 1 — Verifier que le compte existe, est ambassadeur et actif
  let profile;
  try {
    ...
    console.error('[send-reset-ambassador] lecture profiles:', e.message);
    return { statusCode: 500, body: JSON.stringify({ error: 'Lecture impossible' }) };
  }
  if (profile.role !== 'ambassador') return SILENCE;
  ...
  try {
    ...
    if (!res.ok || !data || !data.hashed_token) {
      console.error('[send-reset-ambassador] generate_link:', res.status, JSON.stringify(data).slice(0, 300));
      return { statusCode: 500, body: JSON.stringify({ error: 'Generation du lien impossible' }) };
    }
    hashedToken = data.hashed_token;
  } catch (e) {
    console.error('[send-reset-ambassador] generate_link:', e.message);
    return { statusCode: 500, body: JSON.stringify({ error: 'Generation du lien impossible' }) };
  }
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#03030a;padding:40px 16px;">
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[send-reset-ambassador] resend:', res.status, detail.slice(0, 200));
    }
  } catch (e) {
