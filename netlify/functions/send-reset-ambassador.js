exports.handler = async (event) => {
  ...
  // Sauvegarder le token en base (indispensable pour la validation)
  if (SB_URL && SB_KEY && ambassadorId) {
    ...
    await fetch(`${SB_URL}/rest/v1/ambassador_reset_tokens`, {
      ...
      body: JSON.stringify({ ambassador_id: ambassadorId, token, expires_at: expires, used: false })
    }).catch(() => {});
  }
