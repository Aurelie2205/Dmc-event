...
exports.handler = async () => {
  try {
    ...
    for (const item of posts) {
      ...
      // Copie systématique de tous les champs médias — le type sert à l'affichage,
      // pas à décider quoi transférer. Corrige les carrousels (media_items jamais copié)
      // et les items type='carousel' ne contenant qu'une seule image.
      postObj.image_url = item.image_url || '';
      postObj.video_url = item.video_url || '';
      postObj.embed_url = item.embed_url || (item.type === 'video' ? item.video_url : '') || '';
      postObj.pdf_url = item.pdf_url || '';
      postObj.media_items = item.media_items || null;
      ...
      try {
        await fetch(`${process.env.URL}/.netlify/functions/send-notif`, {
          method: 'POST',
